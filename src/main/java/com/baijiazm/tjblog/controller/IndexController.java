...
import com.baijiazm.tjblog.dto.ErrorCode;
...
import com.baijiazm.tjblog.utils.IPKit;
import com.baijiazm.tjblog.utils.MyUtils;
import com.baijiazm.tjblog.utils.PatternKit;
...
public class IndexController extends BaseController {
    ...
    public String index(HttpServletRequest request, @PathVariable int p, @RequestParam(value = "limit", defaultValue = "12") int limit) {
        ...
        PageInfo<ContentEntity> articles = contentService.getContents(p, limit);
     ...
     * @param contents
     */
    private void completeArticle(HttpServletRequest request, ContentEntity contents) {
        if (contents.getAllowComment()) {
            ...
            PageInfo<CommentEntity> commentsPaginator = commentService.getComments(contents.getId(), Integer.parseInt(cp), 6);
    ...
    public String comment(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap,
        ...
        if (StringUtils.isBlank(ref) || StringUtils.isBlank(_csrf_token)) {
            return errorHint(modelMap, ErrorCode.BAD_REQUEST, "/article/" + cid);
        }
        ...
        if (null == cid || StringUtils.isBlank(text)) {
            return errorHint(modelMap, "请输入完整后评论", "/article/" + cid);
        }

        if (StringUtils.isNotBlank(author) && author.length() > 50) {
            return errorHint(modelMap, "姓名过长", "/article/" + cid);
        }

        if (StringUtils.isNotBlank(mail) && !MyUtils.isEmail(mail)) {
            return errorHint(modelMap, "请输入正确的邮箱格式", "/article/" + cid);
        }

        if (StringUtils.isNotBlank(url) && !PatternKit.isURL(url)) {
            return errorHint(modelMap, "请输入正确的URL格式", "/article/" + cid);
        }

        if (text.length() > 200) {
            return errorHint(modelMap, "请输入200个字符以内的评论", "/article/" + cid);
        }

        String val = IPKit.getIpAddrByRequest(request) + ":" + cid;
        Integer count = cache.hget(Types.COMMENTS_FREQUENCY.getType(), val);
        if (null != count && count > 0) {
            return errorHint(modelMap, "您发表评论太快了，请过会再试", "/article/" + cid);
        }
...
//        }
//
//        PageInfo<ContentEntity> contentsPaginator = contentService.getArticles(metaDto.getMid(), page, limit);
//
...
//        request.setAttribute("archives", archives);
    ...
    public String links(HttpServletRequest request) {
        List<MetaEntity> links = metaService.getMetas(Types.LINK.getType());
    ...
    public String page(@PathVariable String pagename, HttpServletRequest request) {
        ContentEntity contents = contentService.getContents(pagename);
        if (null == contents) {
            return this.render_404();
        }
        if (contents.getAllowComment()) {
    ...
    public String search(HttpServletRequest request, @PathVariable String keyword, @PathVariable int page, @RequestParam(value = "limit", defaultValue = "12") int limit) {
        page = page < 0 || page > WebConst.MAX_PAGE ? 1 : page;
//        PageInfo<ContentEntity> articles = contentService.getArticles(keyword, page, limit);
//        request.setAttribute("articles", articles);
